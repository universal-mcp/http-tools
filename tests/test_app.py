...
@pytest.fixture
def app_instance():
    """Provides a HttpToolsApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return HttpToolsApp(integration=mock_integration)

def test_universal_mcp_http_tools_app_initialization(app_instance):
    """
    Test that the HttpToolsApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "http-tools", "HttpToolsApp instance has unexpected name."


def test_universal_mcp_http_tools_tool_docstrings_format(app_instance):
    """
    Test that each tool method in HttpToolsApp has a well-formatted docstring,
...
def test_universal_mcp_http_tools_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in HttpToolsApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
